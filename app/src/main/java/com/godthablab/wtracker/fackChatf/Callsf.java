...
public class Callsf extends AppCompatActivity {
    LinearLayout msg;
    LinearLayout mute;
    ...
    private class btnEndCallListner implements OnClickListener {
        public void onClick(View view) {
            Callsf callsf = Callsf.this;
            callsf.timeSwapBuff += Callsf.this.timeInMilliseconds;
            Callsf.this.customHandler.removeCallbacks(Callsf.this.updateTimerThread);
            Callsf.this.finish();
        }
    }
    ...
    private class updateTimeThreadListner implements Runnable {
        ...
        public void run() {
            Callsf.this.timeInMilliseconds = SystemClock.uptimeMillis() - Callsf.this.startTime;
            Callsf.this.updatedTime = Callsf.this.timeSwapBuff + Callsf.this.timeInMilliseconds;
            int secs = (int) (Callsf.this.updatedTime / 1000);
            int mins = secs / 60;
            secs %= 60;
            Callsf.this.times.setText("" + String.format("%02d", mins) + " : " + String.format("%02d", secs));
            Callsf.this.customHandler.postDelayed(this, 0);
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getExtras().getInt("ID") == 1) {
            ...
            this.profilename = getIntent().getExtras().getString("NAME");
            if (UserChatf.callImages != null) {
                this.profileImages.setBackground(new BitmapDrawable(getResources(), UserChatf.callImages));
            } else {
                this.profileImages.setBackground(getResources().getDrawable(R.drawable.images));
            }
        }
