...
import static com.godthablab.wtracker.ParentalControl.Utility.ConvertMillisTodateString;
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        try {
            ...
            if (mType.equals("now")) {
                ...
                if (jsonObject.has("offline_date")) {
                    String datetime = ConvertMillisTodateString(jsonObject.getString("offline_time"));
                    holder.offlinedate.setText(datetime.split("_")[0]);
                    holder.offlinetime.setText(datetime.split("_")[1]);
                }
                if (jsonObject.has("online_date")) {
                    String datetime = ConvertMillisTodateString(jsonObject.getString("online_time"));
